...
public class UserDao extends BaseDao{
    ...
    public void insert(User user) {
        String sql = "insert into tb_user (Uuser,Upass) values (?,?)";
    ...
    public boolean IsExistence(String username) {
        String sql = "select * from tb_user where Uuser like ?";
        List<User> list = super.query(sql, new Object[]{username}, User.class);
        return (list!=null&&list.size()>0) ? true : false;
    }
    //根据姓名查询个人信息
    public User findByName(String username){
        String sql = "select * from admin where Uuser like ?";
        List<User> list = super.query(sql, new Object[]{username}, User.class);
        return  (list!=null&&list.size()>0) ? list.get(0) : null;
    }
    //更新个人信息
    public void update(User user) {
        String sql = "update tb_user set Uphone=?,Usex=?,Uemail=?,Uname=? where Uuser like ?";
